...
function Twofish() {
  ...
  function encrypt(userKey, plainText) {
    ...
    var userKey = Buffer.from(userKey);
  ...
  function decrypt(userKey, chiperText) {
    ...
    var userKey = Buffer.from(userKey),
      chiperText = Buffer.from(chiperText, 'hex');
    ...
    cpt = Buffer.from(cpt)
    return cpt.toString('utf8');
  }

  //encrypt cbc
  function encryptCBC(userKey, plainText) {
    ...
    out.data = result.toString('hex');
    out.iv = iv;
    return JSON.stringify(out);
  }

  //decrypt cbc
  function decryptCBC(userKey, chiperText, iv) {
  ...
  return {
    enc: function(userKey, plainText) {
      return encrypt(userKey, plainText);
    },
    encCBC: function(userKey, plainText) {
      return encryptCBC(userKey, plainText).toString('hex');
    },
    dec: function(userKey, chiperText) {
      return decrypt(userKey, chiperText);
    },
